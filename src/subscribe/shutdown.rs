...
pub fn on_sleep_request(_: Arc<Mutex<Client>>, payload: String){
    let sleep_json:Result<Shutdown, serde_json::Error> = serde_json::from_str::<Shutdown>(&payload);
    ...
    let command = std::process::Command::new("shutdown").arg(sleep_command.time.trim()).output();
